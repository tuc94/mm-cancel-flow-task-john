...
export function Modal({
  ...
  const sizeClass =
    size === 'full' ? 'max-w-[96vw]'
    : size === '2xl' ? 'max-w-[1400px]'
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/45 backdrop-blur-[2px]
        ...
        >
          {/* 👇 Always present for Radix’ a11y check */}
          ...
          {/* Your visible header */}
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-black/10
