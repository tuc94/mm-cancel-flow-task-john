  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        {/* Content with scale-up; sticky header; scrollable body */}
        <Dialog.Content
          aria-labelledby={titleId}
          className={`
            fixed left-1/2 top-1/2 z-50 w-[96vw] sm:w-[94vw] ${sizeClass}
        ...
        >
          {/* Header (sticky) */}
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-black/10 bg-white/80 px-5 py-3 backdrop-blur supports-[backdrop-filter]:bg-white/60 sm:px-6 lg:px-8">
            <Dialog.Title id={titleId} className="text-sm font-medium text-gray-800">
              {title}
            </Dialog.Title>
            <Dialog.Close
              aria-label="Close"
